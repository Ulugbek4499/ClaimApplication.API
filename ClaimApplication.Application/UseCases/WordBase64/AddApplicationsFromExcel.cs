...
{
    public record ConvertWordToBase64(IFormFile DocFile) : IRequest<string>;

    public class ConvertWordToBase64Handler : IRequestHandler<ConvertWordToBase64, string>
    {

        private readonly IApplicationDbContext _context;
        private readonly IMapper _mapper;

        public ConvertWordToBase64Handler(IApplicationDbContext context, IMapper mapper)
        {
        ...
        public async Task<string> Handle(ConvertWordToBase64 request, CancellationToken cancellationToken)
        {
            if (request.DocFile == null || request.DocFile.Length == 0)
                throw new ArgumentNullException("File", "file is empty or null");

            var file = request.DocFile;
